library(ggpubr)
# 
plot_list = list(plot_abalone, plot_sonar, plot_eeg, plot_mushrooms,
                 plot_ionosphere, plot_cars, plot_breast_cancer, plot_banknote)#,
                 #plot_sim_60, plot_sim_100, plot_sim_400, plot_sim_1000)

# save(plot_list, file = "results_plots")
# 
#load("results_plots")

#plot_list = list(plot_60, plot_100, plot_400, plot_1000)

plot_list= lapply(plot_list, function(plot){
  plot <- plot + rremove("ylab") #+ rremove("xlab")
    theme(legend.key = element_rect("grey38"), legend.key.size = unit(40,"point"),
          legend.title = element_text(size = 28, face = "bold", family = "TT Arial"),
          legend.text = element_text(size = 22, face = "italic", family = "TT Arial"))
  plot
} )

plot = plot + rremove("ylab") + 
  theme(legend.key = element_rect("grey38"), legend.key.size = unit(30,"point"),
        legend.title = element_text(size = 18, face = "bold", family = "TT Arial"),
        legend.text = element_text(size = 14, face = "italic", family = "TT Arial"))

pub_page = ggarrange(plotlist = plot_list,
             ncol = 4, nrow = 1, common.legend = TRUE, 
                      legend.grob = get_legend(plot),
                  legend = "top") 

  
pub_page

library(grid)
annotate_figure(pub_page, left = textGrob("Y-Axis: Inductive Accuracy", rot = 90, vjust = 1, gp = gpar(cex = 1.3)),
                bottom = textGrob("X-Axis: Number of Pseudo-Labeled Instances", gp = gpar(cex = 1.3)),
                fig.lab.size = 80)



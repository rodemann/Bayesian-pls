...
source("R/bnn_utils.R")
  ...
  standard_self_training_old <- function(labeled_data,
                                     unlabeled_data,
                                     test_data,
                                     target,
                                     glm_formula) {
...
standard_self_training <- function(labeled_data,
                                      ...
                                      target,
                                      glm_formula) {
  # ED: important change #1 - too many noisy features break the model
  ...
  for (i in seq(as.integer(n_imp/10))) {
    ...
    ord <- order(unlist(uncertainty), decreasing = FALSE)
    winner <- ord[1:10]
